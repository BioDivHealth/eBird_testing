# -*- coding: utf-8 -*-
"""2_prompt_eBird_dashboard.ipynb
    ...
    [
        ("system",
         """
          ...
          OBSERVATION DATE = Date of the observation expressed as year-month-day (YYYY-MM-DD).,
          TIME OBSERVATIONS STARTED = The time observations were initiated based on the 24-hour clock.,
          ...
          DURATION MINUTES = The duration of the sampling event reported in minutes.,
...
agent_executor.run(final_prompt.format(
        question="What breeding activities were seen?"
  ))

agent_executor.run(final_prompt.format(
        question="Which observer has the highest overall duration?"
  ))
