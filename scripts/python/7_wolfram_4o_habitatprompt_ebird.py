# -*- coding: utf-8 -*-
"""3_wolfram_4o_habitatprompt_eBird.ipynb
...
# %pip install -qU langchain langchain-openai langchain-community langchain-experimental pandas gradio wolframalpha openai
...
"""Convert to SQL format (make sure google sheets file is mounted)
...
Create a SQL agent to interact with the matrix data"""
...
agent_executor = create_sql_agent(llm, db=db,
                                  ...
                                  verbose=True, tools=tools, memory=memory
)

"""# CSV Version

Initialise the csv agent with zero shot reaction type.
"""
...
agent_executor.run("Convert to a pandas dataframe and then plot a bar chart of habitats?")

"""Build prompt for metadata and further instructions
...
final_prompt = ChatPromptTemplate.from_messages(
    [
        ("system",
         """
          You are a helpful AI assistant expert in querying csv Database to find answers to user's question about bird sightings in Norfolk. Here is some useful metadata for the columns.
          GLOBAL UNIQUE IDENTIFIER = A unique code assigned to each observation ,
          TAXONOMIC ORDER = The numeric value assigned to this taxon to arrange the species in the latest taxonomic sequence.,
          CATEGORY = The category (e.g., species, hybrid, slash) assigned to this taxon.,
          ...
          COMMON NAME = The primary English common name of the bird species taxon.,
          SCIENTIFIC NAME = The scientific (or latin) name of the taxon.,
          SUBSPECIES SCIENTIFIC NAME = The scientific name of the subspecies or subspecies group.,
          ...
          OBSERVER ID = Unique number associated with each observer.,
...
"""Save the chat history in memory

Question samples:
"""

agent_executor.run(final_prompt.format(
        question="How many bird species were seen on the last day in the dataset?"
  ))
...
agent_executor.run(final_prompt.format(
        question="Are weekdays or weekends better for observing different types of birds?"
  ))

agent_executor.run(final_prompt.format(
        question="What was the most unusual observation?"
  ))

"""# Gradio App

Run this to generate the chatbox interface app. 
(Note: for now it's called Seamus bc a much earlier iteration using a PDF file as context started hallucinating and talking about Seamus Heaney...so it stuck)
"""
