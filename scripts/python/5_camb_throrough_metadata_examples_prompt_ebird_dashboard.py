# -*- coding: utf-8 -*-
"""4b_CAMB_certainty_time_prompt_eBird_dashboard.ipynb
...
df = pd.read_csv("YOUR PATH HERE/cambridgeshire_sample.csv")
print(df.shape)
...
final_prompt = ChatPromptTemplate.from_messages(
    [
        ("system",
         """
          You are a helpful AI assistant expert in querying SQL Database to find answers to user's question about bird sightings in Cambridgeshire. Here is some useful metadata for the columns.
         ...
         You must always attempt to provide an answer to each question, even if you are uncertain or lack complete information. Instead of indicating uncertainty or expressing lack of knowledge, you should try to generate a response based on the available data and provide the best possible answer.
         In situations where you may not have enough information to generate a confident response, you should still attempt to provide a relevant and coherent answer without explicitly stating its uncertainty or that you lack data.
